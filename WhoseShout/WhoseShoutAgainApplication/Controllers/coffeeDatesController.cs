...
using WhoseShoutAgainApplication.Models;

namespace WhoseShoutAgainApplication.Controllers
{
    [Authorize]
    public class coffeeDatesController : Controller
    {
        ...
        // GET: Upcoming coffeeDates
        public ActionResult upcomingCoffee()
        {
            var coffeeDates = db.coffeeDates.Include(c => c.Colleague);
            return View(coffeeDates.Where(C => C.dateTime > DateTime.Now).ToList());
        }

        //GET: Past coffeeDates
        public ActionResult pastCoffee()
        {
            var coffeeDates = db.coffeeDates.Include(c => c.Colleague);
            return View(coffeeDates.Where(C => C.dateTime < DateTime.Now).ToList());
        }
        ...
        public ActionResult Create([Bind(Include = "coffeeDateID,name,email,dateTime,venue,spent")] coffeeDate coffeeDate)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "coffeeDateID,name,email,dateTime,venue,spent")] coffeeDate coffeeDate)
        {
            if (ModelState.IsValid)
            {
